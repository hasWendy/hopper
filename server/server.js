...
commander
  .option('-c, --config_path [config_path]',
    'Absolute path to the application config file [/var/has/partnerapp/conf/config.json]',
    '/var/has/partnerapp/conf/config.json')
...
global.conf = {};
conf.settings = require(commander.config_path);
...
conf.settings.environment = envMap[conf.settings.environment] || 'dev';

if (DEV || conf.settings.environment === 'dev') {
  conf.settings.loginEndpoint =
    conf.settings.loginEndpoint ||
...
function setupWinstonLogging() {
  ...
  if (global.DEVELOPMENT || commander.logtoconsole) {
    ...
    var transportOptions = {
      'filename' : conf.settings.logLocation || '/var/has/partnerapp/log/partners_generic.log',
      'maxsize'  : conf.settings.maxLogSize || 25000,
      'maxFiles' : conf.settings.maxLogFiles || 2
    }
...
function initializeServer() {
  ...
  var session_key = conf.settings.session_secret;

  var store = new MemcachedStore({hosts : conf.settings.cache, prefix : 'mat_pub'});
  ...
  app.use(function checkIsInternal(req, res, next) {
    ...
    .use(apiMiddleware(conf.settings.iproxyEndpoint || conf.settings.javascriptEndpoint, 'v2', conf.settings.apiKey));
